...
export default {
  ...
  methods: {
    init() {
      const _this = this;
      if (this.$route.params.alterData) {
        let info = this.$route.params.alterData;
        this.$set(this.addGroup, "title", info.title);
        this.$set(this.addGroup, "level", info.level);
        this.$set(this.addGroup, "groups", info.groups);
        this.$set(this.addGroup, "id", info._id);
        this.selectLevel(info.level-1);
      //      this.groupInfo.forEach((el) => {
      //          console.log("------ds",el)
      //   if (el.name===info.group) {
      //     _this.$set(_this.addGroup, "groupId",el._id);
      //   }
      // });
      }
    },
...
groupIds.push(el._id);
    ...
    commit() {
      this.$store
        .dispatch("education/upQuestionnaire", this.addGroup)
        .then(data => {
          this.$alert("更新成功", {
